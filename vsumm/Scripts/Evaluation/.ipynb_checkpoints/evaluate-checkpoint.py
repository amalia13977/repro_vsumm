...
def main():
	...
	if len(sys.argv)>7 and sys.argv[7]=="1":
		...
		n_clusters=int(percent*frames.shape[0]*sampling_rate/100)+1

	print("Getting frames of summary!")
	frame_indices=[int(idx) for idx in open(directory+'frame_indices_'+ sys.argv[6]+'_'+str(n_clusters)+'_'+str(sampling_rate)+'_5.txt','r').read().splitlines()]
