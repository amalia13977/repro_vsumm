...
def get_resnet_features(frames, n_components_pca=500):
    ...
    if features.shape[1] > n_components_pca: # Only apply PCA if features are higher dimensional
        pca = PCA(n_components=n_components_pca)
        features = pca.fit_transform(features)
        print(f"Applied PCA to ResNet features, new shape: {features.shape}")
...
def main():
    ...
    if "cnn" in method:
        print ("CNN Check!")
        # Pass n_components_pca to reduce feature dimensionality for memory efficiency
        features = get_resnet_features(frames, n_components_pca=500) 
    else:  # vsumm methods
        print ("VSUMM Check!")
    ...
    if "kmeans" in method:
        print ("Kmeans Check!")
        cluster_model = KMeans(n_clusters=num_centroids, random_state=0, n_init=10).fit(features) # Added random_state and n_init for reproducibility and robustness
    else:
        print ("Gaussian Check!")
        cluster_model = GaussianMixture(n_components=num_centroids, random_state=0).fit(features) # Added random_state for reproducibility
    ...
    print(f"DEBUG: Method: {method}, Final frame_indices: {frame_indices}")
